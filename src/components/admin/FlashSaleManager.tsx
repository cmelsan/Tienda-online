...
function getTimeStatus(endTime: string | null): {
  ...
  if (!endTime) return { label: '—', color: 'text-gray-400', isExpired: false, hoursLeft: 0 };
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (expired.length > 0) {
        ...
        addNotification(
          `${expired.length} oferta${expired.length > 1 ? 's' : ''} expirada${expired.length > 1 ? 's' : ''} desactivada${expired.length > 1 ? 's' : ''} automáticamente`,
          'info'
        );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-blue-800 space-y-1">
          <li>✓ Activa productos para la sección Flash Sale de la página de inicio</li>
          <li>✓ Establece el descuento (%) y la hora de finalización</li>
          <li>✓ Las ofertas expiradas se desactivan automáticamente al cargar esta página</li>
              ...
              return (
                <tr key={product.id} className={`${rowBg} transition-colors`}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="font-bold text-gray-800">{(product.price / 100).toFixed(2)} €</div>
                    {product.is_flash_sale && product.flash_sale_discount > 0 && (
                      <div className="text-xs text-rose-600 font-semibold">
                        → {((product.price * (1 - product.flash_sale_discount / 100)) / 100).toFixed(2)} €
                      </div>
                    )}
                  </td>
